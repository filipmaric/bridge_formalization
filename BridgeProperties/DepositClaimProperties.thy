...
lemma reachableGetDepositBridgeNoCancel:
...
next
  ...
  next
    ...
    then show ?thesis using reachable_step
...
lemma reachableNotBridgeDeadNoCancel:
  assumes "reachable contracts contracts' steps"
...
next
  ...
  have "\<not> bridgeDead contracts' tokenDepositAddress"
    using reachableBridgeDead reachable_step.prems
    by (meson reachable.intros(1) reachable.intros(2) reachable_step(2))
  ...
  moreover have "\<nexists>caller amount token proof. CANCEL_WD tokenDepositAddress caller ID token amount proof = step"
    using  callCancelDepositWhileDeadBridgeDead reachable_step.hyps(2) reachable_step.prems
...
lemma reachableGetDepositBridgeNotDead:
  assumes "reachable contracts contracts' steps"
  assumes "\<not> bridgeDead contracts' tokenDepositAddress"
  assumes "getDepositTD contracts tokenDepositAddress ID \<noteq> 0"
  shows "getDepositTD contracts' tokenDepositAddress ID = getDepositTD contracts tokenDepositAddress ID"
  using assms reachableGetDepositBridgeNoCancel reachableNotBridgeDeadNoCancel 
  by meson


text \<open>When the bridge is dead, deposit flags that are zero remain zero\<close>
lemma reachableGetDepositBridgeDead:
  assumes "reachable contracts contracts' steps"
...
next
  ...
  proof (cases step)
    ...
    then show ?thesis
      using reachable_step
      using callDepositOtherAddress callDepositFailsInDeadState reachableDeadState
...
next
  ...
  proof (cases step)
    ...
    then show ?thesis
      using reachable_step
      using callDepositOtherAddress callDepositNotBridgeDead' reachableGetDepositBridgeNotDead
      by (metis (no_types, lifting) executeStep.simps(1) reachable.reachable_step)
  next
    case (CANCEL_WD address' caller' ID' token' amount' proof')
    then show ?thesis
      using reachable_step
  ...
  next
    ...
    proof (cases step)
      ...
      proof (cases "address' \<noteq> tokenDepositAddress")
        case True
        then show ?thesis
          using DEPOSIT reachable_step callDepositOtherAddress[OF True[symmetric]]
          by simp
      next
        case False
        show ?thesis
        proof (cases "ID \<noteq> ID'")
          case True
          then show ?thesis
            using DEPOSIT reachable_step \<open>\<not> address' \<noteq> tokenDepositAddress\<close> callDepositOtherID
            by simp
        next
          case False
          then have "getDepositTD contracts'' tokenDepositAddress ID =
                     hash3 caller' token' amount'"
            using DEPOSIT reachable_step \<open>\<not> address' \<noteq> tokenDepositAddress\<close> callDepositWritesHash
            by auto
          ...
          then have "caller = caller'" "token = token'" "amount = amount'"
            ...
            by blast+
          then show False
            using DEPOSIT reachable_step \<open>\<not> ID \<noteq> ID'\<close>
            by auto
        qed
      qed
    next
    ...
    next
      case (WITHDRAW_WD address' caller' token' amount' proof')
      then show ?thesis
        using reachable_step callWithdrawWhileDeadDeposits callWithdrawWhileDeadOtherAddress
        by (metis executeStep.simps(8) list.set_intros(2))
    qed auto
  qed
qed
...
lemma reachableGetClaimNoClaim:
  assumes "reachable contracts contracts' steps"
...
next
  ...
  proof (cases step)
    case (CLAIM address' caller' ID' token' amount' proof')
    then show ?thesis
      using reachable_step callClaimPreservesTrueClaim callClaimOtherAddress
...
next
  ...
  proof (cases "step = CLAIM bridgeAddress caller ID token amount proof")
    case True
    then show ?thesis
      using reachable_step.hyps callClaimWritesClaim
      by simp
  next
    case False
    then show ?thesis
      using reachable_step
      by (metis reachable.simps reachableGetClaim set_ConsD)
  qed
qed
...
theorem noDepositBridgeDead: 
  ...
  using assms callDepositFailsInDeadState reachableBridgeDead
  by blast
...
lemma noDepositAfterGetDepositNonzero:
  ...
  shows False
  using assms
  by (smt (verit, best) executeStep.simps(1) callDepositFailsInDeadState callDepositWrittenHash fst_conv reachableGetDepositBridgeNotDead reachableStepInSteps)
...
proof (cases "bridgeDead contracts'' address")
  ...
  then have "getDepositTD contracts'' address ID \<noteq> 0"
    using False reachableGetDepositBridgeNotDead hash3_nonzero[of "sender msg" token amount] assms
    by auto
  then show ?thesis
...
proof-
  ...
  then have "getClaimB contracts'' address ID = True"
    using \<open>reachable contracts' contracts'' steps\<close>
    unfolding state'_def
    using reachableGetClaim
    by blast
  then show ?thesis
...
lemma DEPOSITNoDoubleCons:
  assumes "reachable contracts contracts' (DEPOSIT tokenDepositAddress caller ID token amount # steps)"
  shows "\<nexists> token' caller' amount'. DEPOSIT tokenDepositAddress caller' ID token' amount' \<in> set steps"
  by (smt (verit) assms callDepositNoDouble executeStep.simps(1) fst_conv reachableCons' reachableStepInSteps)
...
lemma DEPOSITNoDouble:
  ...
  shows False
  using assms
  by (metis DEPOSITNoDoubleCons Un_iff append_Cons list.set_intros(1) reachableAppend set_append)
...
lemma CLAIMNoDoubleCons:
  assumes "reachable contracts contracts' (CLAIM bridgeAddress caller ID token amount proof # steps)"
...
proof-
  obtain contracts'' where *: 
    ...
    "reachable contracts contracts'' (CLAIM bridgeAddress caller ID token amount proof # (steps2 @ [CLAIM bridgeAddress caller' ID token' amount' proof'] @ steps3))"
...
proof-
  ...
  obtain steps1 steps2 contractsU contractsU' blockU blockNumU where *:
   "reachable contractsD' contractsU steps2"
   "executeStep contractsU blockU blockNumU ?update = (Success, contractsU')"
   "reachable contractsU' contractsC steps1"
    using reachableStepInSteps[OF \<open>reachable contractsD' contractsC steps\<close> update]
    by (smt (verit, ccfv_threshold) assms(3) in_set_conv_decomp_last reachableAppend reachableCons' update)
  ...
  obtain contractsUx contractsU'x blockx blockNumx steps1x steps2x stateRootx where *:
    "reachable contractsD' contractsUx steps1x"
    ...
    "reachable contractsU'x contractsC steps2x"
    "\<nexists>stateRoot'. UPDATE stateOracleAddress stateRoot' \<in> set steps2x"
    using lastUpdateHappened'[OF \<open>reachable contractsD' contractsU steps2\<close> ** \<open>reachable contractsU' contractsC steps1\<close>]
    by metis
  ...
  proof
    show "properSetup contractsUx tokenDepositAddress bridgeAddress"
      using \<open>reachable contractsD' contractsUx steps1x\<close> assms(1) deposit by auto
  next
    show "let stateOracleAddress = stateOracleAddressB contractsUx bridgeAddress
      ...
      by simp
  next
    show "reachable contractsU'x contractsC steps2x"
      by fact
  next
  ...
  have "hash3 (sender msg) token amount = hash3 (sender msg') token' amount'"
    ...
    using hash3_nonzero
    using \<open>reachable contractsD' contractsUx steps1x\<close>
...
proof-
  ...
  have "lastState (the (stateOracleState contractsI stateOracleAddress)) \<noteq> 0"
    by (metis getLastStateBContractsUNonZero reachableBridgeStateOracle reachableInitI stateOracleAddress_def)
  ultimately
  obtain contractsU contractsU' block blockNum steps1 steps2 stateRoot' where *:
    "reachable contractsInit contractsU steps1"
    ...
    "reachable contractsU' contractsI steps2"
    "\<nexists>stateRoot'. UPDATE stateOracleAddress stateRoot' \<in> set steps2"
    "set steps1 \<subseteq> set stepsInit"
    using lastUpdateHappened[OF reachableInitI, of stateOracleAddress]
    by auto
  ...
  proof
    show "properSetup contractsU tokenDepositAddress bridgeAddress"
      using \<open>reachable contractsInit contractsU steps1\<close> assms(1) 
      by auto
  next
    show "let stateOracleAddress = stateOracleAddressB contractsU bridgeAddress
      ...
      by (metis "*"(1) "*"(3) reachableBridgeStateOracle stateOracleAddress_def)
  next
    show "reachable contractsU' contractsI steps2"
      by fact
  next
    show "let stateOracleAddress = stateOracleAddressB contractsU' bridgeAddress
           in \<nexists>stateRoot'. UPDATE stateOracleAddress stateRoot' \<in> set steps2"
      by (metis "*"(4) "*"(5) reachableBridgeStateOracle reachableInitI stateOracleAddress_def)
  qed
  ...
  have "getDepositTD contractsU tokenDepositAddress ID = hash3 (sender msg) token amount"
    ...
    by (smt (verit, best) reachableBridgeStateOracle callUpdateITokenDeposit properSetupReachable stateOracleAddress_def)
  then show ?thesis
    using getDepositWrittenOnlyByDeposit
    using depositsEmpty assms
    using \<open>reachable contractsInit contractsU steps1\<close> \<open>set steps1 \<subseteq> set stepsInit\<close>
    by blast
qed
...
lemma UpdateBetweenDepositAndClaim:
  ...
  assumes "reachable contractsI contracts steps" 
...
proof-
  interpret Init: Init where contractsI = contracts and stepsInit = "steps @ stepsInit"
    using Init'_axioms Init_axioms.intro Init_def assms(2) reachableTrans by auto
  ...
  obtain contractsD' contractsD contractsC' contractsC where reach:
   "reachable contractsI contractsD' steps1"
   "reachable contractsD' contractsD [?DEPOSIT_step]"
   "reachable contractsD contractsC' steps2"
   "reachable contractsC' contractsC [?CLAIM_step]"
    using assms
  ...
  have **: "reachable contractsInit contractsC' (tl steps @ stepsInit)"
    by (smt (verit, ccfv_threshold) Cons_eq_appendI list.sel(3) reach(1) reach(2) reach(3) reachableInitI reachableTrans self_append_conv2 steps)

  obtain contracts' steps1' steps2' stateRoot
    where "reachable contractsInit contracts' steps1'" 
  ...
  reach': "reachable contracts' contractsC' (steps2' @ [UPDATE ?soAddress stateRoot])"
  ...
  then obtain contracts1 blockU blockNumU where
    ...
    "reachable contracts1 contractsC' steps2'"
    by (metis executeStep.simps(6) reachableAppend reachableSingleton)
  ...
  proof
    show "properSetup contracts' tokenDepositAddress bridgeAddress"
      using \<open>reachable contractsInit contracts' steps1'\<close> by auto
  next
    show "let stateOracleAddress = stateOracleAddressB contracts' bridgeAddress
           in callUpdate contracts' stateOracleAddress blockU blockNumU stateRoot =
              (Success, contracts1)"
      by (metis HashProofVerifier.reachableBridgeStateOracle HashProofVerifier_axioms \<open>callUpdate contracts' (stateOracleAddressB contractsInit bridgeAddress) blockU blockNumU stateRoot = (Success, contracts1)\<close> \<open>reachable contractsInit contracts' steps1'\<close>)
  next
    show "reachable contracts1 contractsC' steps2'"
      by fact
  next
    show "let stateOracleAddress = stateOracleAddressB contracts1 bridgeAddress
           in \<nexists>stateRoot'. UPDATE stateOracleAddress stateRoot' \<in> set steps2'"
      using \<open>\<nexists>stateRoot'. UPDATE (stateOracleAddressB contractsInit bridgeAddress) stateRoot' \<in> set steps2'\<close> \<open>callUpdate contracts' (stateOracleAddressB contractsInit bridgeAddress) blockU blockNumU stateRoot = (Success, contracts1)\<close> \<open>reachable contractsInit contracts' steps1'\<close> by auto
  qed

  have getDeposit: "getDepositTD contracts' tokenDepositAddress ID = hash3 caller' token' amount'"
    using LU.callClaimGetDeposit
    by (metis executeStep.simps(2) reach(4) reachableSingleton senderMessage)
...
proof-
  ...
  obtain contractsC' contractsC where CC':
     ...
     "reachable contractsC contractsC' [CLAIM bridgeAddress caller ID token amount proof]"
    using bl
  ...
  have "DEPOSIT tokenDepositAddress caller ID token amount \<in> set (steps1' @ [last stepsInit])"
    ...
    by (metis executeStep.simps(2) reachableSingleton senderMessage)
  then show ?thesis
    using bl assms
...
proof-
  ...
  have reach: "reachable contractsInit contractsDeposit (DEPOSIT_step # stepsInit)"
    using reach
    using DEPOSIT_step_def reachableTrans by fastforce
  ...
  obtain steps1 steps2 c1 c2 blockNum block where *:
    ...
    "reachable c2 contractsDeposit steps1"
    "DEPOSIT_step # stepsInit = steps1 @ [CLAIM_step] @ steps2"
    using reachableStepInSteps reach CLAIM_in_steps
...
proof-
  ...
  obtain contracts where C: "reachable contractsInit contracts ([?claimStep] @ steps1)"
  ...
  then show False
    using assms 
    by (metis DEPOSITNoDouble' Un_iff reachableInitI set_append)
qed
